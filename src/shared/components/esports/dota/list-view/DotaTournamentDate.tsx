...
function DotaTournamentDate({
  tournament,
  showTournamentsMatches,
  selectTournament
}: Props) {
  return showTournamentsMatches ? (
    <Row onClick={selectTournament(tournament.id)}>
      <Col>
        {tournament.league && tournament.league.name}{" "}
        {tournament.series && tournament.series.name} {tournament.name}
      </Col>
    </Row>
  ) : (
    <Row onClick={selectTournament(tournament.id)}>
